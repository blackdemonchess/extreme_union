...
class WebsocketClient(object):
    ...
    def __init__(self, api):
        ...
        """
        self.api = api
        self.global_id = self.api.global_id
    ...
    @staticmethod
    def dict_queue_add(dicio, maxdict, key1, key2, key3, value):
        if key3 in dicio[key1][key2]:
            dicio[key1][key2][key3] = value
        else:
            while True:
                try:
                    dic_size = len(dicio[key1][key2])
                except:
                    dic_size = 0
                if dic_size < maxdict:
                    dicio[key1][key2][key3] = value
                    ...
                    # del mini key
                    del dicio[key1][key2][sorted(
                        dicio[key1][key2].keys(), reverse=False)[0]]

    @staticmethod
    def api_dict_clean(obj):
        if len(obj) > 5000:
            for k in obj.keys():
                del obj[k]
                break
    ...
    def on_message(self, message):  # pylint: disable=unused-argument
        ...
        global_value.ssl_Mutual_exclusion[self.global_id] = True
        ...
        #######################################################
            ...
            if self.api.profile.msg != False:
                ...
                try:
                    ...
                    pass
                # Set Default account
                if global_value.balance_id[self.global_id
                ] is None:
                    for balance in message["msg"]["balances"]:
                        if balance["type"] == 4:
                            global_value.balance_id[self.global_id
                            ] = balance["id"]
                            break
                try:
    ...
    def on_error(wss, error):  # pylint: disable=unused-argument
        ...
        logger = logging.getLogger(__name__)
        logger.error(error)
        global_value.websocket_error_reason['abc'] = str(error)
        global_value.check_websocket_if_error['abc'] = True
    ...
    def on_open(wss):  # pylint: disable=unused-argument
        ...
        global_value.check_websocket_if_connect['abc'] = 1
    ...
    def on_close(wss):  # pylint: disable=unused-argument
        ...
        global_value.check_websocket_if_connect['abc'] = 0
