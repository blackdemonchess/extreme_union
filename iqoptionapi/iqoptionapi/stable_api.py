...
class IQ_Option:
    ...
    def connect(self):
        ...
        self.api = IQOptionAPI(
            "iqoption.com", self.email, self.password, self.global_id)
        ...
        if check == True:
            ...
            while global_value.balance_id[self.global_id] is None:
                pass

            self.position_change_all(
                "subscribeMessage", global_value.balance_id[self.global_id])
    ...
    def get_currency(self):
        ...
        for balance in balances_raw["msg"]:
            if balance["id"] == global_value.balance_id[self.global_id]:
                return balance["currency"]

    def get_balance_id(self):
        return global_value.balance_id[self.global_id]
    ...
    def change_balance(self, Balance_MODE):
        def set_id(b_id):
            if global_value.balance_id[self.global_id] is not None:
                self.position_change_all(
                    "unsubscribeMessage", global_value.balance_id[self.global_id])

            global_value.balance_id[self.global_id] = b_id
    ...
    def buy(self, price, ACTIVES, ACTION, expirations, sleep_time=0.05):
        try:
            ...
            req_id = str(len(self.api.buy_multi_option))
        try:
            self.api.buy_multi_option[req_id] = {'id': None}
        except:
            pass
