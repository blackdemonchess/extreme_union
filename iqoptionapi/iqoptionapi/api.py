...
class IQOptionAPI(object):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, host, username, password, global_id, proxies=None):
        """
    ...
    def send_websocket_request(self, name, msg, request_id="", no_force_send=True):
        ...
        while (global_value.ssl_Mutual_exclusion[self.global_id] or global_value.ssl_Mutual_exclusion_write[self.global_id]) and no_force_send:
            pass
        global_value.ssl_Mutual_exclusion_write[self.global_id] = True
        self.websocket.send(data)
        global_value.ssl_Mutual_exclusion_write[self.global_id] = False
...
# -------------------------------------------------------
    ...
    def start_websocket(self):
        global_value.check_websocket_if_connect[self.global_id] = None
        global_value.check_websocket_if_error[self.global_id] = False
        global_value.websocket_error_reason[self.global_id] = None
        ...
        while True:
            try:
                if global_value.check_websocket_if_error[self.global_id]:
                    return False, global_value.websocket_error_reason[self.global_id]
                if global_value.check_websocket_if_connect[self.global_id] == 0:
                    return False, "Websocket connection closed."
                elif global_value.check_websocket_if_connect[self.global_id] == 1:
                    return True, None
            except:
                pass
    ...
    def send_ssid(self):
        ...
        self.ssid(global_value.SSID[self.global_id])  # pylint: disable=not-callable
    ...
    def connect(self):
        global_value.balance_id[self.global_id] = None
        global_value.ssl_Mutual_exclusion[self.global_id] = False
        global_value.SSID[self.global_id] = None
        global_value.ssl_Mutual_exclusion_write[self.global_id] = False
        ...
        if global_value.SSID[self.global_id] != None:
            ...
            if check_ssid == False:
                ...
                try:
                    global_value.SSID[self.global_id] = response.cookies["ssid"]
                except:
        ...
        requests.utils.add_dict_to_cookiejar(
            self.session.cookies, {"ssid": global_value.SSID[self.global_id]})
